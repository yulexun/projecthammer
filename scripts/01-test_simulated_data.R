...
dataset <- read_parquet("data/00-simulated_data/simulated_data.parquet")
...
test_that("Dataset structure and values", {
  ...
  expect_true(all(c(
    ...
    "current_price", "old_price", "units", "price_per_unit_numeric"
  ) %in% colnames(dataset)))
  ...
  expect_false(any(is.na(dataset$old_price)))
